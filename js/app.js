 ...
 */
    ...
    (() => {
        ...
        function rangeInit() {
            ...
            if (priceSlider) {
                ...
                priceSlider.noUiSlider.on("update", (function(values) {
                    ...
                    sumCount.textContent = Number(values[0]);
                    inputSum.value = Number(values[0]);
                }));
            }
            ...
            if (priceSlider2) {
                ...
                priceSlider2.noUiSlider.on("update", (function(values) {
                    const timeCount = document.querySelector("._time-count");
                    const inputTimem = document.querySelector("#input-time");
                    let declination = "лет";
                    if (Number(values[0]) == 1) declination = "год"; else if (Number(values[0]) == 2 || Number(values[0]) == 3 || Number(values[0]) == 4) declination = "года";
                    timeCount.textContent = Number(values[0]) + " " + declination;
                    inputTimem.value = Number(values[0]);
                }));
            }
        }
        rangeInit();
        ...
        function initSliders() {
            ...
            if (document.querySelector(".loans__slider")) new swiper_core_Swiper(".loans__slider", {
                modules: [ Pagination, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 4,
        ...
        if (subMenuBtn.length > 0) subMenuBtn.forEach((item => {
            item.addEventListener("click", (function(e) {
                e.preventDefault();
            }));
        }));
        const scrollLinks = document.querySelectorAll("._scroll");
        if (scrollLinks.length > 0) scrollLinks.forEach((link => {
            const itemSelector = "#" + link.href.split("#")[1];
            const itemGo = document.querySelector(itemSelector);
            link.dataset.goto = itemSelector;
            if (!itemGo) {
                link.removeAttribute("data-goto");
                link.href = "/" + itemSelector;
            }
        }));
        document.addEventListener("click", (function(e) {
            ...
            if (isMobile.any()) if (e.target.closest(".menu-item-has-children")) {
                const subtn = e.target.closest(".menu-item-has-children");
                if (subtn.classList.contains("_active")) subtn.classList.remove("_active"); else subtn.classList.add("_active");
            } else subMenuBtn.forEach((item => {
